<template>
    ...
    <VueScrollFixedNavbar>
      <b-navbar toggleable="lg"  class="nav-bar">    
        <router-link tag="b-navbar-brand" :to="{name:'main'}" style="font-weight: 1000; color: rgb(231, 146, 55)">Recipe Site</router-link>
        <!-- Home -->
        <b-navbar-nav>
        ...
        <!-- Search -->
        <b-navbar-nav>
          <router-link tag="b-nav-item" :to="{name:'search'}"><b style="color: rgb(21, 19, 19)">Search</b></router-link >
        </b-navbar-nav>
        <!-- About -->
        <b-navbar-nav>
        ...
        <b-navbar-nav v-if="$root.store.username">
          ...
          <b-nav-item-dropdown text="Personal" class="bold-option">
            <router-link tag="b-dropdown-item" :to="{name:'favorites'}"><b style="color: rgb(0, 0, 0)" >Favorites</b></router-link>     
            <router-link tag="b-dropdown-item" :to="{name:'myrecipes'}"><b style="color: rgb(0, 0, 0)1, 20, 20)">My Recipes</b></router-link>
            <router-link tag="b-dropdown-item" :to="{name:'familyrecipes'}"><b style="color: rgb(0, 0, 0)">My Family Recipes</b></router-link>
          </b-nav-item-dropdown>
        </b-navbar-nav>
        <!-- Hello guest dropdown: Register & Login -->
        <b-navbar-nav class="ml-auto">
          <b-nav-item-dropdown v-if="!$root.store.username" right class="bold-option" text="Hello guest" style="color:rgb(0, 0, 0) ;">
            <router-link tag="b-dropdown-item" :to="{name:'register'}"><b style="color:rgb(64, 131, 49) ;">Register</b></router-link>
            <router-link tag="b-dropdown-item" :to="{name:'login'}"><b style="color:rgb(81, 173, 69), 4, 4) ;">Login</b></router-link>
          </b-nav-item-dropdown>
          <!-- Logout -->
          <span v-else>
            <b-nav-item>
              <b-row> 
                <b-col>
                  <b-navbar-item><h3 style="color:rgb(143, 8, 8) ;">Hello {{$root.store.username}}</h3></b-navbar-item>  
                </b-col> 
  ...
  .nav-bar{
    ...
    z-index: 10;
    font-family: cursive;
    background-color: rgba(245, 232, 211, 0.658)!important;
    height:50px ;
